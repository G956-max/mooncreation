...
import { doc, getDoc, getDocs, collection, onSnapshot, query, where, orderBy, addDoc, serverTimestamp, updateDoc } from 'firebase/firestore';
...
export default function ProductDetail() {
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Fetch all reviews to calculate dynamic stats
      const reviewsQ = query(collection(db, 'reviews'), where('productId', '==', product?.id));
      const snap = await getDocs(reviewsQ);
      const list = snap.docs.map(doc => doc.data() as { rating: number });
      
      const newReviewsCount = list.length;
      const totalStars = list.reduce((sum, r) => sum + r.rating, 0);
      const newAverageRating = newReviewsCount > 0 ? parseFloat((totalStars / newReviewsCount).toFixed(1)) : 5.0;
  ...
  const accordionItems = [
    ...
    },
    {
      ...
      content: 'Free standard shipping on orders over ₹999. Returns accepted within 30 days of delivery in original packaging.'
    }
  ];

  return (
    <div className="bg-[#FAF9F6] min-h-screen pb-12 md:pb-20">
      {/* Breadcrumb */}
      <div className="w-full px-4 sm:px-6 lg:px-8 py-3 md:py-6">
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-20">
          ...
          <div className="w-full lg:w-1/2 space-y-4">
            <div className="aspect-square rounded-2xl md:rounded-[32px] overflow-hidden bg-white shadow-sm border border-gray-100 relative group">
              <img 
              ...
              <div className="absolute top-4 right-4 md:top-6 md:right-6 flex flex-col gap-2">
                <button 
                  ...
                  className="p-2 md:p-3 bg-white/80 backdrop-blur-md rounded-full text-gray-400 hover:text-red-500 transition-all shadow-sm" 
                  title={isInWishlist(product.id) ? "Remove from Wishlist" : "Add to Wishlist"}
                >
                  <Heart size={16} className={isInWishlist(product.id) ? "fill-red-500 text-red-500" : ""} />
                </button>
                <button onClick={handleShare} className="p-2 md:p-3 bg-white/80 backdrop-blur-md rounded-full text-gray-400 hover:text-[#2C2C2C] transition-all shadow-sm" title="Share Product">
                  <Share2 size={16} />
                </button>
              </div>
            </div>
          ...
          <div className="w-full lg:w-1/2 space-y-5 md:space-y-8">
            <div className="space-y-3 md:space-y-4">
              <div className="flex items-center gap-1.5 md:gap-2 text-[10px] md:text-sm font-bold text-gray-400 uppercase tracking-widest">
                ...
                {reviews.length > 0 ? (
                  ...
                  <span className="text-xs text-gray-400 bg-white border border-gray-200 px-2.5 py-0.5 rounded-full font-medium">No ratings yet</span>
                )}
              </div>
              ...
              <p className="text-xl md:text-3xl font-bold text-[#2C2C2C]">₹{product.price}</p>
                ...
                <div className="flex flex-wrap gap-2 md:gap-3">
                  {product.options.values.map((val) => (
                    <button
                      ...
                      className={`px-4 py-2 md:px-6 md:py-3 rounded-xl md:rounded-2xl font-bold text-xs md:text-sm transition-all border-2 ${
                        selectedOption === val 
            ...
            <div className="space-y-4 md:space-y-6 pt-2 md:pt-4">
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 md:gap-6">
                <div className="flex items-center justify-between bg-white rounded-xl md:rounded-2xl border border-gray-100 p-1 shadow-sm w-full sm:w-auto">
                  <button 
                    ...
                    className="p-2 md:p-3 text-gray-400 hover:text-black transition-colors"
                  >
                    <Minus size={16} />
                  </button>
                ...
                <div className="flex-1 flex gap-2 md:gap-4">
                  <button 
                    ...
                    className="flex-grow bg-white text-[#2C2C2C] border-2 border-[#2C2C2C] py-2.5 md:py-4 rounded-xl md:rounded-2xl font-bold hover:bg-gray-50 text-xs md:text-sm transition-all"
                  >
                    Add to Cart
                  </button>
                  <button 
                    onClick={handleBuyNow}
                    className="flex-grow bg-[#2C2C2C] text-white py-2.5 md:py-4 rounded-xl md:rounded-2xl font-bold hover:bg-black text-xs md:text-sm transition-all shadow-lg md:shadow-xl shadow-black/10"
                  >
                    Buy Now
                  </button>
                </div>
              ...
              <div className="flex items-center justify-between py-3 md:py-4 border-y border-gray-100 text-[10px] md:text-sm text-gray-500 font-medium gap-1">
                <div className="flex items-center gap-1 md:gap-2">
                  <Truck size={14} className="text-gray-400 md:w-5 md:h-5" />
                  <span>Free Shipping</span>
                </div>
                <div className="flex items-center gap-1 md:gap-2">
                  <RefreshCcw size={14} className="text-gray-400 md:w-5 md:h-5" />
                  <span>30-Day Returns</span>
                </div>
                <div className="flex items-center gap-1 md:gap-2">
                  <ShieldCheck size={14} className="text-gray-400 md:w-5 md:h-5" />
              ...
              {accordionItems.map((item) => (
                <div key={item.id} className="border-b border-gray-100">
                  <button
                    ...
                    className="w-full flex items-center justify-between py-3 md:py-5 text-left group"
                  >
                    <span className="font-serif font-bold text-sm md:text-lg text-[#2C2C2C] group-hover:text-black transition-colors">
                      {item.title}
                    </span>
                    {openAccordion === item.id ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                  </button>
                  <div className={`overflow-hidden transition-all duration-300 ${
                    openAccordion === item.id ? 'max-h-40 pb-3 md:pb-5' : 'max-h-0'
                  }`}>
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 mt-12 md:mt-24">
        <div className="border-t border-gray-200 pt-8 md:pt-16">
          <div className="flex items-center gap-2 md:gap-3 mb-6 md:mb-10">
            <MessageSquare className="text-[#2C2C2C] w-6 h-6 md:w-7 md:h-7" />
            <h2 className="text-lg md:text-3xl font-serif font-bold text-[#2C2C2C]">Customer Reviews</h2>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            {hasOrdered && (
              <div className="bg-white p-4 md:p-8 rounded-2xl md:rounded-3xl border border-gray-100 shadow-sm h-fit sticky top-24">
                <h3 className="text-base md:text-xl font-bold text-[#2C2C2C] mb-4 md:mb-6">Write a Review</h3>
