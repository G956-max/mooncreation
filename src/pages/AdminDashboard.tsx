  ...
  useEffect(() => {
    ...
    const unsubscribeOrders = onSnapshot(ordersQ, (snapshot) => {
      const list = snapshot.docs.map(doc => {
        const data = doc.data();
        return {
          id: `#ORD-${doc.id.slice(0, 8).toUpperCase()}`,
          customer: data.customerName || 'Anonymous',
          date: data.createdAt?.toDate ? data.createdAt.toDate().toLocaleDateString() : 'Recent',
          status: data.status || 'Pending',
          amount: `₹${(data.pricing?.total || data.total || 0).toLocaleString()}`
        };
      });
      setRecentOrders(list);
    }, (error) => {
      console.warn("Index not ready for dashboard orders query:", error);
      const fallbackQ = query(collection(db, 'orders'), limit(5));
