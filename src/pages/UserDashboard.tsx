                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="space-y-4">
                    {loadingOrders ? (
                      ...
                      <div className="text-center py-12 bg-[#FAF9F6] rounded-3xl border border-dashed border-gray-200">
                        <p className="text-gray-500">You haven't placed any orders yet.</p>
                        <Link to="/categories" className="inline-block mt-4 bg-[#2C2C2C] text-white px-6 py-2.5 rounded-xl font-bold text-xs hover:bg-black transition-all">
                          Start Shopping
                        </Link>
                      </div>
                    ) : (
                      userOrders.map((order) => (
                        <div key={order.id} className="p-6 bg-[#FAF9F6] rounded-3xl border border-gray-100 flex flex-col gap-6 hover:border-gray-200 transition-all">
                          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-gray-200/60 pb-4 gap-4">
                            <div>
                              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Order ID</p>
                              <p className="text-sm font-bold text-blue-600">#ORD-{order.id.slice(0, 8).toUpperCase()}</p>
                            </div>
                            <div>
                            ...
                            <div>
                              ...
                              <p className="text-sm font-medium text-gray-600">{order.paymentMethod}</p>
                            </div>
                            <div>
                            ...
                            {order.items?.map((item: any, idx: number) => (
                              <div key={idx} className="flex items-center gap-4">
                                ...
                                <div className="flex-1 min-w-0">
                                  <h4 className="font-serif font-bold text-sm text-[#2C2C2C] truncate">{item.name}</h4>
                          ...
                          <div className="flex justify-between items-center pt-4 border-t border-gray-200/60">
                            <div>
                              ...
                              <p className="text-lg font-bold text-[#2C2C2C]">₹{(order.pricing?.total || order.total || 0).toLocaleString()}</p>
                            </div>
                            {order.status === 'Delivered' && (
                              <Link 
                                to={`/product/${order.items?.[0]?.id}`}
                                className="bg-[#2C2C2C] text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-black transition-colors"
                              >
                                Write a Review
                              </Link>
                            )}
