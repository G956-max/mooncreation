...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { 
      name: 'Occasion', 
      path: '#', 
      dropdown: ['Birthday Gifts', 'Anniversary Gifts', 'Wedding Gifts', 'Baby Shower Gifts', "Valentine's Day Gifts", "Mother's Day Gifts", "Father's Day Gifts", 'Friendship Day Gifts', 'Graduation Gifts'] 
    },
    { 
      name: 'Personalized Gifts', 
      path: '#', 
      dropdown: ['Photo Gifts', 'Name Printed Gifts', 'Customized Chocolates', 'Customized Frames', 'Photo Lamps', 'Name Wallets', 'Customized Mugs', 'Spotify Frames', 'Photo Pillows']
    },
    { 
      name: 'Gifts by Person', 
      path: '#', 
      dropdown: ['Gifts for Him', 'Gifts for Her', 'Gifts for Husband', 'Gifts for Wife', 'Gifts for Boyfriend', 'Gifts for Girlfriend', 'Gifts for Kids', 'Gifts for Friends', 'Gifts for Parents']
    },
    { 
      name: 'Combo Gifts', 
      path: '#', 
      dropdown: ['Chocolate Combo', 'Couple Combo', 'Surprise Box Combo', 'Birthday Combo', 'Love Combo']
    },
    { 
      name: 'Customized Studio', 
      path: '#', 
      dropdown: ['Customized Frames', 'Customized Chocolates']
    },
  ];
  ...
  return (
    <>
      <nav className="sticky top-0 z-50 bg-white border-b border-gray-100 shadow-sm">
        ...
        <div className="hidden md:block w-full px-4 sm:px-8 lg:px-12 border-b border-gray-50">
          <div className="flex justify-between items-center h-20 gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              >
                <HomeIcon size={22} className="text-gray-600 group-hover:text-[#C48B22]" />
                <span className="text-[10px] font-bold text-gray-500 uppercase mt-1">{t('home')}</span>
              </button>
        ...
        <div className="md:hidden w-full bg-white px-4 py-3">
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => navigate('/')} className="p-2 relative hidden xs:block">
                <HomeIcon size={22} className="text-[#2C2C2C]" />
              </button>
              <button onClick={handleProfileClick} className="p-2 relative hidden xs:block">
                <User size={22} className="text-[#2C2C2C]" />
              </button>
        ...
        {/* Desktop Bottom Row */}
        <div className="hidden md:block w-full px-4 sm:px-8 lg:px-12 bg-white border-t border-gray-50 overflow-visible">
            ...
            {navLinks.map((link, index) => (
              <div key={index} className="relative group h-full flex items-center">
                <Link 
                  ...
                  className={`text-sm font-bold tracking-wide transition-colors flex items-center gap-1 ${(location.pathname === link.path && link.path !== '#') ? 'text-[#C48B22]' : 'text-[#2C2C2C] hover:text-[#C48B22]'}`}
                >
                  {link.name}
                  {link.dropdown && <ChevronDown size={14} className="text-gray-400 group-hover:text-[#C48B22] transition-transform group-hover:rotate-180" />}
                </Link>
                {link.dropdown && (
                  <div className="absolute top-full left-0 hidden group-hover:block z-50">
                    <div className="bg-white border border-gray-100 shadow-xl rounded-xl py-2 min-w-[220px] flex flex-col mt-1">
                      {link.dropdown.map((dropItem, idx) => {
                        const isCustomStudio = link.name === 'Customized Studio';
                        const dropPath = isCustomStudio 
                          ? (dropItem === 'Customized Frames' ? '/product/custom-frame' : '/product/custom-chocolate')
                          : `/category/${encodeURIComponent(dropItem)}`;
                        return (
                          <Link 
                            key={idx} 
                            to={dropPath}
                            className="px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-[#C48B22] transition-colors"
                          >
                            {dropItem}
                          </Link>
                        );
                ...
                {navLinks.map((link, index) => (
                  <div key={index} className="flex flex-col">
                    <Link 
                      ...
                      className="flex justify-between items-center p-4 bg-gray-50 rounded-xl font-bold text-sm"
                    >
                      {link.name}
                      {link.dropdown && (
                        <ChevronDown 
                    ...
                    {link.dropdown && openMobileDropdown === link.name && (
                      <div className="flex flex-col gap-2 mt-2 ml-4 pl-4 border-l-2 border-gray-100">
                        {link.dropdown.map((dropItem, idx) => {
                          const isCustomStudio = link.name === 'Customized Studio';
                          const dropPath = isCustomStudio 
                            ? (dropItem === 'Customized Frames' ? '/product/custom-frame' : '/product/custom-chocolate')
                          ...
                          return (
                            <Link
                              ...
                              className="py-2 text-sm font-medium text-gray-600 hover:text-[#C48B22]"
                            >
                              {dropItem}
                            </Link>
                          );
